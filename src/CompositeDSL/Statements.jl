...
function _dsl_parse_context_write_assignment(alias_map, outputs::Tuple, rhs, known_outputs::Set{Symbol}, state_outputs::Set{Symbol})
    ...
    value_expr = _dsl_rewrite_alias_expr(alias_map, inner, protected_symbols)
    return (
        kind = :entity,
        spec_expr = :(Processes.ContextWrite($(QuoteNode(output)), $value_expr)),
        alias_name = Symbol(),
        positional_specs = (),
...
function _dsl_build_statement(stmt, alias_map, context_map, known_outputs::Set{Symbol}, state_outputs::Set{Symbol}, expected_schedule::Symbol, owner_name::Symbol; include_condition = nothing)
    ...
    context_write = _dsl_parse_context_write_assignment(alias_map, outputs, rhs, known_outputs, state_outputs)
